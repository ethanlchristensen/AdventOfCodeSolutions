...
class Solution:
    ...
    def part1(self):
        ...
        while True:
            new_position, new_data = self.get_next_position(
    ...
    def part2(self):
        ...
        for y in range(len(self.data)):
            for x in range(len(self.data[0])):
                if self.placement_sees_other_blockers(
                    x,
                    y,
                ):
                    ...
                    new_data = [
                        [self.data[j][i] for i in range(len(self.data[0]))]
                        for j in range(len(self.data))
                    ]
                    new_data[y][x] = "#"

                    if self.is_loop(
                        guard_position=guard_position, total_chars=total_chars
                    ):
                        loops += 1
    ...
    def get_next_position(self, position):
        ...
        if not 0 <= new_position[0] < len(self.data[0]) or not 0 <= new_position[
            1
        ] < len(self.data):
    ...
    def is_loop(self, guard_position, total_chars, debug=False):
        ...
        while True:
            ...
            if (
                ...
                and self.data[new_position[1]][new_position[0]] == "^"
            ):
                return True
