...
class Solution:
    ...
    def load_data(self, name="data"):
        with open(name, "r") as file:
            ...
            registers = [
                re.search(r"Register (.)\: (\d+)", line).groups() for line in lines[:-1]
            ]
    ...
    def part1(self):
        ...
        registers, program = self.load_data("self.data")
    ...
    def part2(self):
        ...
        while self.run_program(value, program) == program:
            value -= 1

        print(value + 1)

        res = self.run_program(value + 1, program)
        print(program)
        print(res)
    ...
    def get_ranges(self, n, nb):
        step = n / nb
        return [(round(step * i), round(step * (i + 1))) for i in range(nb)]

    def run_program(self, n, program):
        output = []
        while n != 0:
            n = n >> 3
            output.append(n % 8)
    ...
    def correct_digits(self, result, program):
        c = 0
