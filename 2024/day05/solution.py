...
class Solution:
    ...
    def load_data(self, name="data"):
        ...
        file.close()
        rules = [
            [int(v) for v in match.group().split("|")]
            for match in re.finditer(pattern=r"\d+\|\d+", string=self.data)
        ]
        pages = [
            [int(v) for v in match.group().split(",")]
            for match in re.finditer(pattern=r"\d+(,\d+)*,\d+", string=self.data)
        ]
        return rules, pages
    ...
    def part1(self):
        ...
        for page in pages:
            ...
            if not self.failed_rules(page, rules):
            ...
            if self.failed_rules(page, rules):
                ...
                while failures := self.failed_rules(page, rules):
                    for failed_rule in failures:
                        ...
                        page[page.index(failed_rule[0])] = page[
                            page.index(failed_rule[1])
                        ]
