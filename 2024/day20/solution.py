...
class Solution:
        ...
        def bfs(board, sx, sy):
            ...
            while len(q) > 0:
                ...
                for dx, dy in [(0, 1), (1, 0), (-1, 0), (0, -1)]:
                    nx, ny = x + dx, y + dy
                    if (
                        0 <= nx < len(self.data[0])
                        and 0 <= ny < len(self.data)
    ...
    def part2(self):
        ...
        def bfs(board, sx, sy, max_cheat_duration=0):
            seen = set()
            q = queue(
                [(sx, sy, [], max_cheat_duration)]
            )  # (x, y, path, remaining cheat time)
            ...
            while q:
                ...
                for dx, dy in [(0, 1), (1, 0), (-1, 0), (0, -1)]:
                    ...
                    if (
                        0 <= nx < len(self.data[0])
                        and 0 <= ny < len(self.data)
                        and (nx, ny, cheat_time) not in seen
                    ):
                        if (
                            board[ny][nx] == "#" and cheat_time > 0
                        ):  # Wall, but we can cheat through it
    ...
    def print_path(self, s, e, path):
        ...
        for row in self.data:
            print(
                "".join(row)
                .replace(".", "  ")
                .replace("#", bc("  ", None, 255).colored)
                .replace("A", bc("  ", None, 208).colored)
                .replace("B", bc("  ", None, 125).colored)
            )
