...
class Solution:
    ...
    def part1(self):
        ...
        def check_dir(s, x, y, dir, debug_tracker=None):
            ...
            if s == "XMAS":
                ...
                if debug_tracker:
                    print(f"s={s} is goated for being XMAS!")
                lock_x, lock_y = x, y
                locks[y][x] = s[-1]
                for _ in range(2, 5):
                    lock_x += -dir[0]
            ...
            if debug_tracker:
                print(
                    f"Recieved: ({x}, {y}), {s} --> Checking: ({x + dir[0]}, {y + dir[1]}), {self.data[y + dir[1]][x + dir[0]]}"
                )
            ...
            if not 0 <= y + dir[1] < len(self.data) or not 0 <= x + dir[0] < len(
                self.data[0]
            ):
                if debug_tracker:
                    print("erm, we outta bounds pal.")
                return 0

            # is it possible our string could still be XMAS?
            if s in "XMAS":
                if debug_tracker:
                    print(f"'{s}' is in XMAS, lets look some more!")
                return check_dir(
                    s + self.data[y + dir[1]][x + dir[0]],
                    x + dir[0],
                    y + dir[1],
                    dir,
                ...
                for dir in directions:
                    if (
                        ...
                        total += check_dir(
                            ...
                            y=y,
                            dir=dir,
                            debug_tracker=1 if debug else None,
                        )
                    else:
                        total += check_dir(
                            s=self.data[y][x], x=x, y=y, dir=dir, debug_tracker=None
                        )
    ...
    def part2(self):
        ...
        def star_out(x, y):
            ...
            if (
                self.data[y + d1[0][1]][x + d1[0][0]] == "M"
                and self.data[y + d1[1][1]][x + d1[1][0]] == "S"
            ) or (
                self.data[y + d1[0][1]][x + d1[0][0]] == "S"
                and self.data[y + d1[1][1]][x + d1[1][0]] == "M"
            ):
                # check d2 - does this second diagonal make 'MAS'?
                if (
                    self.data[y + d2[0][1]][x + d2[0][0]] == "M"
                    and self.data[y + d2[1][1]][x + d2[1][0]] == "S"
                ) or (
                    self.data[y + d2[0][1]][x + d2[0][0]] == "S"
                    and self.data[y + d2[1][1]][x + d2[1][0]] == "M"
                ):
                    # add to the locks array for printing out later
                    locks[y][x] = self.data[y][x]
                    locks[y + d1[0][1]][x + d1[0][0]] = self.data[y + d1[0][1]][
                        x + d1[0][0]
                    ]
                    locks[y + d1[1][1]][x + d1[1][0]] = self.data[y + d1[1][1]][
                        x + d1[1][0]
                    ]
                    locks[y + d2[0][1]][x + d2[0][0]] = self.data[y + d2[0][1]][
                        x + d2[0][0]
                    ]
                    locks[y + d2[1][1]][x + d2[1][0]] = self.data[y + d2[1][1]][
                        x + d2[1][0]
                    ]
                    return 1
            return 0
