...
class Solution:
    ...
    def part1(self):
        ...
        disk_layout = self.get_layout(self.data)
        ...
        total = sum(
            [int(char) * idx for idx, char in enumerate(disk_layout) if char != "."]
        )
        end = time.time()
    ...
    def part2(self):
        ...
        lowest_id, highest_id = (
            min([v for v in disk_layout if v != "."]),
            max([v for v in disk_layout if v != "."]),
        )
        while True:
            open_blocks = self.get_open_blocks(disk_layout=disk_layout)
            next_block = self.get_right_most_block(
                disk_layout=disk_layout,
                seen_block_values=seen_values,
                highest=highest_id,
            )
            ...
            for block in open_blocks:
                if (
                    ...
                    and block["start"] < next_block["start"]
                ):
    ...
    def get_open_blocks(self, disk_layout):
        blocks = []
    ...
    def get_right_most_block(self, disk_layout, seen_block_values, highest):
        ...
        for idx in range(len(disk_layout) - 1, -1, -1):
            if (
                ...
                and disk_layout[idx] != "."
                and (
                    (not seen_block_values and disk_layout[idx] == str(highest))
                    or disk_layout[idx] == str(int(seen_block_values[-1]) - 1)
                )
            ):
                block["end"] = idx
                block["value"] = disk_layout[idx]
            if block["end"] != -1 and (
                disk_layout[idx] == "." or disk_layout[idx] != block["value"]
            ):
                block["start"] = idx + 1
