...
class Solution:
    ...
    def part1(self):
        ...
        self.data = [(list(line[0]), int(line[1])) for line in self.data]
        ...
        df = pd.DataFrame()
        df["hand"] = ["".join(line[0]) for line in self.data]
        df["card_1_val"] = [strength[line[0][0]] for line in self.data]
        df["card_2_val"] = [strength[line[0][1]] for line in self.data]
        df["card_3_val"] = [strength[line[0][2]] for line in self.data]
        df["card_4_val"] = [strength[line[0][3]] for line in self.data]
        df["card_5_val"] = [strength[line[0][4]] for line in self.data]
        df["hand_type"] = hand_assignments
        df["hand_type_val"] = [
            hand_assignments_value[hand_assignment]
            for hand_assignment in hand_assignments
        ]
        df["bid"] = [line[1] for line in self.data]
        df = df.sort_values(
            by=[
                "hand_type_val",
                "card_1_val",
    ...
    def solve(self):
        ...
        """
        part_one_answer, time_to_get = self.part1()
        part_two_answer, time_to_get = self.part2()
